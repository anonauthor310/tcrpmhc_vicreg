#!/usr/bin/env python3
"""Raw ESMC VICReg training loop.

Core model, loss and scoring definitions live in src/models/. This module
loads frozen esmc_300m shards, trains the projection heads, selects the
checkpoint and exports latents.

  1) esm_vicreg: VICReg heads on raw ESMC embeddings
  2) pretrained_esmc_meanpool: frozen meanpool baseline on the same shards

score = -mean((z_T - z_P)^2). Higher is more binder-like.
"""
...
class RawRunConfig:
    ...
    checkpoint_root: str = str(REPO / "outputs/checkpoints/esm_vicreg_raw_complete")
    output_root: str = str(REPO / "outputs/esm_vicreg_raw_complete")
    figure_root: str = str(REPO / "figures/esm_vicreg_raw_complete")

    pretrained_embed_root: str = str(REPO / "data/embeddings/raw_esmc_300m")
    pretrained_immrep_shard_dir: str = str(REPO / "data/embeddings/raw_esmc_300m/immrep_test")
