...
@dataclass
class ExportConfig:
    train_csv: str = str(REPO / "data/processed/train.csv.gz")
    val_csv: str = str(REPO / "data/processed/val.csv.gz")
    test_csv: str = str(REPO / "data/processed/test.csv.gz")
    immrep_csv: str = str(REPO / "data/processed/immrep_test.csv.gz")
    embed_root: str = str(REPO / "data/embeddings/lora_esmc_300m")
    checkpoint_dir: str = str(REPO / "models/checkpoints")
    batch_size: int = 8
    num_workers: int = 0
    chunk_size: int = 100
    dtype: str = "float16"
...
def load_split_sequences(
    ...
    if missing:
        raise ValueError(
            f"Missing columns in {csv_path}: {missing}. Available: {list(df.columns)}"
        )
...
def load_lora_encoders(
    ...
    ckpt_map = {
        "tcr": checkpoint_dir / "tcr_encoder_checkpoint.pth",
        "pep": checkpoint_dir / "peptide_encoder_checkpoint.pth",
        "hla": checkpoint_dir / "hla_encoder_checkpoint.pth",
    }
    ...
    tcr_ckpt = torch.load(ckpt_map["tcr"], map_location="cpu")
    pep_ckpt = torch.load(ckpt_map["pep"], map_location="cpu")
    hla_ckpt = torch.load(ckpt_map["hla"], map_location="cpu")

    tcr_encoder.load_state_dict(tcr_ckpt["tcr_model_state_dict"])
    peptide_encoder.load_state_dict(pep_ckpt["peptide_model_state_dict"])
    hla_encoder.load_state_dict(hla_ckpt["hla_model_state_dict"])
    ...
    print("LoRA encoders loaded (eval mode, no optimizer state).", flush=True)
    return tcr_encoder, peptide_encoder, hla_encoder, tokenizer
...
def save_embedding_shards_for_split(
    ...
    for i, (tcr_batch, pep_batch, hla_batch) in enumerate(
        zip(tcr_loader, pep_loader, hla_loader)
    ):
        ...
        emb_T = cast_embeddings(tcr_encoder(sequence_tokens=tcr_ids).embeddings, dtype).cpu()
        emb_P = cast_embeddings(peptide_encoder(sequence_tokens=pep_ids).embeddings, dtype).cpu()
        emb_H = cast_embeddings(hla_encoder(sequence_tokens=hla_ids).embeddings, dtype).cpu()
    ...
    print(
        f"{split_name}: saved {summary['n_shards']} shard(s) | "
        f"rows={summary['n_rows']} | dir={split_dir}",
        flush=True,
    )
    return summary
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--train-csv", default=ExportConfig.train_csv)
    p.add_argument("--val-csv", default=ExportConfig.val_csv)
    p.add_argument("--test-csv", default=ExportConfig.test_csv)
    p.add_argument("--immrep-csv", default=ExportConfig.immrep_csv)
    p.add_argument("--embed-root", default=ExportConfig.embed_root)
    p.add_argument("--checkpoint-dir", default=ExportConfig.checkpoint_dir)
    p.add_argument("--batch-size", type=int, default=ExportConfig.batch_size)
    p.add_argument("--num-workers", type=int, default=ExportConfig.num_workers)
    p.add_argument("--chunk-size", type=int, default=ExportConfig.chunk_size)
    p.add_argument("--dtype", choices=("float16", "float32"), default=ExportConfig.dtype)
    ...
    p.add_argument("--lora-r", type=int, default=ExportConfig.lora_r)
    p.add_argument("--lora-alpha", type=int, default=ExportConfig.lora_alpha)
    p.add_argument("--lora-dropout", type=float, default=ExportConfig.lora_dropout)
    p.add_argument("--verify", action="store_true", help="Verify shard keys/shapes after writing.")
    p.add_argument("--verify-only", action="store_true", help="Only verify an existing embed root.")
    return p.parse_args()
...
def main() -> None:
    ...
    if not embed_root.is_absolute():
        embed_root = REPO / embed_root
    ...
    run_config_path = embed_root / "export_lora_esmc_run_config.json"
    with open(run_config_path, "w") as f:
        json.dump(asdict(cfg), f, indent=2)

    print("=" * 80, flush=True)
    print("LoRA ESMC shard export", flush=True)
    ...
    if args.verify_only:
        report = verify_shards(embed_root, splits)
        report_path = embed_root / "export_lora_esmc_verify_report.json"
    ...
    tcr_encoder, peptide_encoder, hla_encoder, tokenizer = load_lora_encoders(
    ...
    for split_name in splits:
        ...
        if not Path(csv_path).exists():
            raise FileNotFoundError(f"CSV for split '{split_name}' not found: {csv_path}")
    ...
    manifest_path = embed_root / "export_lora_esmc_manifest.json"
