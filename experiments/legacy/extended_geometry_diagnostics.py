#!/usr/bin/env python3
"""Extended / historical geometry diagnostics. Not the paper Figure 2 path.

Figure 2b is ``experiments/same_peptide_recovery.py``.
Figure 2d is ``experiments/multicognate_retrieval.py``.
This module keeps the frequency-capped protocol, protocol-comparison plots,
and mAP/MRR retrieval tables.
...
FIG_DIR = REPO / "figures/paper_analysis/refined_geometry"
...
def tcr_seed_offset(tcr: str) -> int:
    digest = hashlib.sha256(str(tcr).encode("utf-8")).digest()
    ...
    for seed in seeds:
        ...
        for model, (zT, zPH) in models.items():
            ...
            for tcr, cognates in multi.items():
                ...
                rng_q = np.random.default_rng(seed + tcr_seed_offset(tcr))
...
def main() -> None:
    ...
    if args.skip_retrieval:
        ...
        print("Paper figure: PYTHONPATH=. python figures/make_figure2.py --split test", flush=True)
        return
