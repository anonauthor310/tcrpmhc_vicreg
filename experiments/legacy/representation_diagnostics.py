#!/usr/bin/env python3
"""Extended representation diagnostics. Not the paper Figure 2c path.

Figure 2c is ``experiments/neighbour_enrichment.py``.
This module keeps CKA, Procrustes, effective rank, nuisance correlations,
t-SNE, score-distribution plots, and the broader cross-reactivity tables.

Representation diagnostics, including neighbour enrichment.

Figure 2(c) is the same-peptide k-nearest-neighbour enrichment for
k in {5, 10, 20}: same-peptide purity in each TCR's neighbourhood divided by the
purity expected from peptide prevalence in the eligible candidate pool, so 1.0
means a neighbourhood no more peptide-pure than chance. TCRs are grouped by
**peptide sequence irrespective of MHC** (as in panel (b)); only panel (d) ranks
explicit peptide-MHC complexes. That table is written to
``results/paper_analysis/crossreactivity/knn_peptide_purity.csv`` and consumed by
``figures/make_figure2.py``.

This script is read-only with respect to training outputs. It also consolidates
metrics, computes ablation deltas, and runs the remaining representation
diagnostics from saved prediction CSVs and latent NPZ files.

Expected folder layout:
  outputs:     REPO/outputs/{run_name}/seed_{seed}/
  figures:     REPO/figures/{run_name}/seed_{seed}/
  checkpoints: REPO/outputs/checkpoints/{run_name}/seed_{seed}/best.pt
...
Diagnostics produced:
  ...
  4. Cross-reactivity: within- vs between-peptide distances, and the kNN
     same-peptide purity/enrichment table used by Figure 2(c).
  5. Nuisance correlations with TCR / peptide / MHC lengths.
  6. t-SNE qualitative TCR-space plots for top peptides.

The representation diagnostics require {split}_latents.npz files. If these are
missing, rerun training with --save-latents (the paper configs set it).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--outputs-root", default=str(REPO / "outputs"))
    parser.add_argument("--figures-root", default=str(REPO / "figures"))
    parser.add_argument("--analysis-out-dir", default=str(REPO / "results/paper_analysis"))
    parser.add_argument("--analysis-fig-dir", default=str(REPO / "figures/paper_analysis"))
