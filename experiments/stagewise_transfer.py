#!/usr/bin/env python3
"""Stagewise transfer diagnostic (input / pre-expander / final latent).

Extracts fixed-input, pre-expander and final-latent representations for TCR,
peptide, MHC and concatenated pMHC from validation-selected checkpoints.
Compares internal test vs IMMREP for one-hot, raw ESMC and LoRA ESMC VICReg
over seeds 31, 37, 43, 49 and 55. Does not retrain.
"""
...
TRAINERS = REPO / "src" / "trainers"
OUT_DIR = REPO / "results/paper_analysis/immrep_transfer_stage_diagnostic"
FIG_DIR = REPO / "figures/paper_analysis/immrep_transfer_stage_diagnostic"
...
# Canonical processed splits used for all stagewise comparisons.
CANONICAL_CSVS = {
    "train": REPO / "data/processed/train.csv.gz",
...
oh = _load_module("train_onehot_vicreg_workshop", TRAINERS / "onehot.py")
esm_common = _load_module("esm_vicreg_common", TRAINERS / "esm_vicreg_common.py")
esm_ft = _load_module("train_esm_vicreg_workshop", TRAINERS / "lora_esmc.py")
esm_raw = _load_module("train_esm_vicreg_raw_workshop", TRAINERS / "raw_esmc.py")


MODELS = {
    "onehot_vicreg": {
        "label": "One-hot + VICReg",
        "ckpt_root": REPO / "outputs/checkpoints/onehot_vicreg_complete",
        "out_root": REPO / "outputs/onehot_vicreg_complete",
        "score_col": "onehot_vicreg_score",
        "family": "onehot",
    },
    "raw_esmc_vicreg": {
        "label": "Raw ESMC + VICReg",
        "ckpt_root": REPO / "outputs/checkpoints/esm_vicreg_raw_complete",
        "out_root": REPO / "outputs/esm_vicreg_raw_complete",
        "score_col": "esm_vicreg_score",
        "family": "raw_esmc",
    },
    "lora_esmc_vicreg": {
        "label": "LoRA ESMC + VICReg",
        "ckpt_root": REPO / "outputs/checkpoints/esm_vicreg_finetuned_complete",
        "out_root": REPO / "outputs/esm_vicreg_finetuned_complete",
        "score_col": "esm_vicreg_score",
        "family": "lora_esmc",
    },
}
...
def verify_scores(extracted: dict, model_key: str, seed: int, split: str) -> dict:
    """Compare extracted final scores to saved predictions on overlapping pair_ids."""
    if split == "train":
        return {
            "verified": True,
