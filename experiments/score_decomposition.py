#!/usr/bin/env python3
"""Score-decomposition diagnostic reported in Appendix F.

Computes internal-test and IMMREP AUROCs for negative MSE, TCR norm,
dot product and cosine similarity from saved validation-selected latents.
Uses paper seeds 31, 37, 43, 49 and 55; no retraining.

Optional extras (not run by default):
  --per-peptide         per-peptide IMMREP forest plot
  --pmhc-permutation    inference-time pMHC-unit derangement diagnostic
"""
...
DEFAULT_OUT = REPO / "results/paper_analysis/immrep_failure_analysis"
DEFAULT_FIG = REPO / "figures/paper_analysis/immrep_failure_analysis"
...
def load_latents(model_key: str, seed: int, split: str, hla_lookup: pd.DataFrame) -> Dict:
    ...
    path = REPO / "outputs" / cfg["run"] / f"seed_{seed}" / f"{split}_latents.npz"
    if not path.exists():
...
- test: `data/processed/test.csv.gz`
- IMMREP: `data/processed/immrep_test.csv.gz`
...
def main() -> None:
    ...
    parser.add_argument(
        "--per-peptide",
        action="store_true",
        help="Also write per-peptide IMMREP diagnostics and forest plot.",
    )
    parser.add_argument(
        "--pmhc-permutation",
        action="store_true",
        help="Additional unreported diagnostic: derange pMHC units at inference time.",
    )
    ...
    paper_path = REPO / "results" / "score_decomposition.csv"
    ...
    perm = None
    if args.pmhc_permutation:
        print("=== 3) Inference-time pMHC permutation (unreported diagnostic) ===", flush=True)
