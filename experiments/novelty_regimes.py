...
Outputs:
  results/paper_analysis/category_breakdown/
...
DEFAULT_MODELS = {
    "onehot_composition": {
        "root": REPO / "outputs/onehot_vicreg_complete",
        "score_col": "onehot_composition_score",
    },
    "pretrained_esmc_meanpool": {
        "root": REPO / "outputs/esm_vicreg_raw_complete",
        "score_col": "pretrained_esmc_meanpool_score",
    },
    "finetuned_esmc_meanpool": {
        "root": REPO / "outputs/esm_vicreg_finetuned_complete",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--val-csv",
        default=str(REPO / "data/processed/val.csv.gz"),
        help="Val split used by the current VICReg runs (occurrence-matched).",
    )
    p.add_argument(
        "--test-csv",
        default=str(REPO / "data/processed/test.csv.gz"),
        help="Test split used by the current VICReg runs (occurrence-matched).",
    )
    p.add_argument(
        "--out-dir",
        default=str(REPO / "results/paper_analysis/category_breakdown"),
    )
